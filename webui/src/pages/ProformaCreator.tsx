...
export default function ProformaCreator() {
  // Wizard state
  ...
  const loadTemplates = useCallback(async () => {
    ...
    try {
      const response = await templates.list({ template_type: "proforma", active_only: true });
  ...
  // Build request data
  const buildRequest = (): CreateProformaRequest => {
  ...
  const handlePreview = async () => {
    setPreviewing(true);
    try {
      const blob = await creators.previewProforma(buildRequest());
  ...
  const handleGenerate = async () => {
    setGenerating(true);
    try {
      const result = await creators.createProforma(buildRequest());
      setSnackbar({
        open: true,
        message: result.message || "Proforma invoice created successfully",
        severity: "success",
      });
      // Download the PDF
      if (result.document?.id) {
        ...
        link.href = url;
        link.download = `proforma-${documentNumber}.pdf`;
      ...
      setSnackbar({
        open: true,
        message: err instanceof Error ? err.message : "Failed to generate proforma",
        severity: "error",
      });
    } finally {
      setGenerating(false);
